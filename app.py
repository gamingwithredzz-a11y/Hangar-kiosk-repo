...
def pacific_time(timestamp):
    dt = datetime.fromtimestamp(timestamp, timezone.utc).astimezone(PACIFIC)
...
def init_db():
    with db() as conn:
        ...
        ensure_column(conn, "kitchen_tickets", "claimed_by_name", "text")
        ensure_column(conn, "kitchen_tickets", "claimed_at", "integer")
        ensure_column(conn, "kitchen_tickets", "completed_by", "text")
        ensure_column(conn, "kitchen_tickets", "completed_by_name", "text")
        ensure_column(conn, "kitchen_tickets", "completed_at", "integer")
...
def get_or_create_cart(conn, table_id, avatar_id, avatar_name):
    ...
    if cart:
        if avatar_name:
            conn.execute(
                "update carts set avatar_name = ? where id = ?",
                (avatar_name, cart["id"]),
            )
        return cart["id"]
    ...
    conn.execute(
        """insert into carts
        (id, table_id, avatar_id, avatar_name, status, created_at)
        values (?, ?, ?, ?, 'cart', ?)""",
...
def cart_items(conn, cart_id):
    return rows_to_dicts(conn.execute(
        "select item_id, name, price_linden from cart_items where cart_id = ?",
        (cart_id,),
    ))

def create_ticket(conn, bill):
    existing = conn.execute(
        "select * from kitchen_tickets where bill_id = ?",
        (bill["id"],),
    ).fetchone()
    ...
    conn.execute("""
        ...
        (id, bill_id, table_id, avatar_id, avatar_name, items,
         amount_linden, status, claimed_by, claimed_by_name, claimed_at,
    ...
    return dict(conn.execute(
        "select * from kitchen_tickets where id = ?",
        (ticket_id,),
    ).fetchone())
...
def api_menu(_query, _body):
    with db() as conn:
        items = rows_to_dicts(conn.execute(
            "select * from menu_items where available = 1 order by rowid"
        ))
...
def api_cart_add(_query, body):
    ...
    with db() as conn:
        ...
        conn.execute("""
            ...
            (id, cart_id, item_id, name, price_linden)
            values (?, ?, ?, ?, ?)
        """, (
            make_id("ci"),
            cart_id,
...
def api_order_place(_query, body):
    ...
    with db() as conn:
        pending = conn.execute("""
            select * from bills
            where table_id = ? and avatar_id = ?
            and status = 'pending_payment'
        """, (table_id, avatar_id)).fetchone()
        ...
        conn.execute(
            "update carts set status = 'pending_payment' where id = ?",
            (cart["id"],),
        )

        conn.execute("""
            insert into bills
            (id, payment_id, cart_id, table_id, avatar_id, avatar_name,
             amount_linden, status, created_at)
...
def api_pay_pending(query, _body):
    ...
    with db() as conn:
        bills = rows_to_dicts(conn.execute("""
            select payment_id, table_id, avatar_id, amount_linden, status
            from bills
            where avatar_id = ? and status = 'pending_payment'
        """, (avatar_id,)))
...
def api_pay_confirm(_query, body):
    ...
    with db() as conn:
        bill = conn.execute(
            "select * from bills where payment_id = ?",
            (payment_id,),
        ).fetchone()
...
def serialize_ticket(ticket):
    ...
    ticket["created_at_pacific"] = pacific_time(ticket["created_at"])

    if ticket.get("claimed_at"):
        ticket["claimed_at_pacific"] = pacific_time(ticket["claimed_at"])
    else:
        ticket["claimed_at_pacific"] = None

    if ticket.get("completed_at"):
        ticket["completed_at_pacific"] = pacific_time(ticket["completed_at"])
    else:
        ticket["completed_at_pacific"] = None

    # Never expose avatar IDs to the kitchen board/controller unless needed.
    ticket.pop("avatar_id", None)
    ticket.pop("claimed_by", None)
    ticket.pop("completed_by", None)
    return ticket

def api_tickets(_query, _body):
    with db() as conn:
        tickets = conn.execute(
            "select * from kitchen_tickets order by created_at desc"
        ).fetchall()

    return ok({"timezone": "America/Los_Angeles", "tickets": [
        serialize_ticket(ticket) for ticket in tickets
    ]})

def api_claim(_query, body):
    ticket_id = body.get("ticket_id")
    staff_avatar_id = body.get("staff_avatar_id")
    staff_avatar_name = body.get("staff_avatar_name") or staff_avatar_id

    if not ticket_id or not staff_avatar_id:
        return err("ticket_id and staff_avatar_id are required")
    ...
    with db() as conn:
        ...
        if ticket["status"] == "complete":
            return err("ticket is already complete", 409)

        if ticket["claimed_by"] and ticket["claimed_by"] != staff_avatar_id:
            return err(
                f"ticket is already claimed by {ticket['claimed_by_name'] or 'another staff member'}",
                409,
            )
    ...
    return ok({"ticket": serialize_ticket(updated)})
...
def api_complete(_query, body):
    ...
    with db() as conn:
        ...
        if ticket["claimed_by"] and ticket["claimed_by"] != staff_avatar_id:
            return err(
                f"only {ticket['claimed_by_name'] or 'the claiming staff member'} can complete this ticket",
                403,
            )
...
def api_clear_completed(_query, _body):
    with db() as conn:
        count = conn.execute(
            "select count(*) as count from kitchen_tickets where status = 'complete'"
        ).fetchone()["count"]

        conn.execute(
            "delete from kitchen_tickets where status = 'complete'"
        )
...
#clock{font-size:15px;color:#c9bba8}
main{padding:18px;display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px}
...
.meta{color:#c9bba8;font-size:13px;line-height:1.55}
.claimed{margin-top:8px;color:#f0b45f;font-weight:700}
...
function itemText(t){return t.items.map(i=>i.name).join(", ");}
...
async function load(){
  document.getElementById("clock").textContent =
    ...
    dateStyle:"medium",timeStyle:"medium"}).format(new Date())+" PT";
  ...
  board.innerHTML=tickets.map(t=>`
    <section class="ticket">
      <div class="top">
        <div class="table">TABLE ${t.table_id}</div>
        <div class="status">${t.status}</div>
      </div>
      <div class="items">${itemText(t)}</div>
      <div class="meta">
        Guest: ${t.avatar_name||"Guest"}<br>
        Amount: L$${t.amount_linden}<br>
        Received: ${t.created_at_pacific}
      </div>
      ${t.claimed_by_name?`<div class="claimed">Claimed by: ${t.claimed_by_name}<br>Claimed: ${t.claimed_at_pacific}</div>`:""}
      <p>
...
ROUTES = {
    ...
    ("POST", "/api/kitchen/clear-completed"): api_clear_completed,
}
